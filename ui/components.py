        ...
        with st.container():
            ...
            with col3:
                ...
                if st.session_state.get(confirm_key, False):
                    ...
                    # Show delete button
                    if st.button("🗑️ Delete", key=delete_key, type="secondary", help="Delete this image"):
                        st.session_state[confirm_key] = True
                        st.rerun()
                
                # Show warning message when in confirmation mode
                if st.session_state.get(confirm_key, False):
                    st.warning(f"⚠️ Delete **{entry.file_name}**?")
            ...
            # Metadata tabs
            tab1, tab2, tab3, tab4 = st.tabs(["🏷️ Labels & Text", "📷 Camera Data", "📊 Technical", "🗂️ Details"])
